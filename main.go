...
// listenForHotkey should run as a go routine since it locks to the OS thread to allow it to have a fixed
// thread listening to Windows API messages
func listenForHotkey(hotkey api.Hotkey, onHotKeyPressed func()) {
	api.RegisterHotKey(hotkey, onHotKeyPressed)
}
...
func setupTray(doShow func(), doQuit func()) {
	...
	systray.SetTitle("GoAnywhere")
	systray.SetTooltip("GoAnywhere")
